...
const BasicScreen = ({
    ...
    const fetchVitalName = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/Screening/Screening_sub_list/?screening_list=${selectedNameId}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
  ...
  const handleTabClick = (tabName, basicScreeningPkId) => {
    if (
      ...
      tabName === "Female Child Screening"
    ) {
      return;
    }
                ...
                <div className="content contenttype" id="generalContent">
                  ...
                  {selectedTab === "Treatment" && (
                    <Treatment
                      pkid={pkid}
                      citizensPkId={citizensPkId}
                      basicScreeningPkId={basicScreeningPkId}
                      scheduleID={scheduleID}
                      citizenidddddddd={citizenidddddddd}
