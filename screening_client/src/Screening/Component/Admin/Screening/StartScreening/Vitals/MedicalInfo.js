...
const MedicalInfo = ({
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${API_URL}/Screening/citizen_medical_history/`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${API_URL}/Screening/citizen_past_operative_history/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchDataById = async (pkid) => {
      try {
        const response = await fetch(
          `${API_URL}/Screening/medical_get_api/${pkid}/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            const pastOperativeData =
              screeningInfo.past_operative_history || [];
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      <Dialog open={openDialog} onClose={handleDialogCancel}>
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to submit this medical form?
          </Typography>
        </DialogContent>
        <DialogActions>
